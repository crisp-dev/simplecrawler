 ...
 * @param  {Boolean} [manualCrawl] If true, then next crawl is called manually.
 * @return {Crawler} Returns the crawler instance to enable chained API calls
 */
Crawler.prototype.start = function(manualCrawl) {
    var crawler = this;
        ...
        process.nextTick(function() {
            if (manualCrawl !== true) {
                crawler.crawlIntervalID = setInterval(
                    crawler.crawl.bind(crawler), crawler.interval
                );
            }
...
Crawler.prototype.getRequestOptions = function(queueItem) {
    ...
    if (queueItem.protocol === "https" && crawler.ignoreInvalidSSL) {
